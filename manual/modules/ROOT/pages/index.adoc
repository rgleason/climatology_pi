...
=== How do I set the Wind Rose settings so that they work right? 
Set the two settings somewhere around Size:100 and Spacing:100 or Size:125 and Spacing:60.
...
=== How to set the Wind settings for a more useful graphical display?

This display is very useful for seeing the general wind patterns and strength over a large area. The wind atlas roses show the percentage of time the wind is in each direction, the arrows show the direction and strength of the average wind, and the overlay map shows the strength of the average wind. This display is very useful for planning longer voyages and for visualizing weather patterns.

. Wind Atlas Roses with size and distance apart set for screen size
. Overlay Map enabled showing wind strength with opacity set highish.
. Direction Arrows enabled, with barbs, size 10 and spacing 36. Opacity set to 25%.
. Under the wind tab, set the size and distance. For me with a high res display size is 200 spacing 170 set on max opacity.

image::wind-atlas-spacing-size-on-high-res-screens.jpg[]

image::wind-atlas-settings.jpg[]

Nice display which shows what you need to know on the macro scale.
