...
// SetPluginMessage
// Parses GRIB’s JSON timeline
// Converts to wxDateTime
// Converts to climatology month index
// Updates ClimatologyRenderParams
// Calls ApplyRenderParams()
// ApplyRenderParams updates:
// StandardDisplayParams
// CycloneParams
//  UI controls (timeline slider, etc.)
// ClimatologyRenderParams
//  triggers PushParamsToFactoryAndRender()
// Grib drives the pipeline
//  GRIB → ClimatologyRenderParams → Dialog UI → StandardDisplayParams → RenderParams → Factory → Render
...
void climatology_pi::SetPluginMessage(wxString &message_id, wxString &message_body)
{
    if (message_id == "GRIB_TIMELINE")
    {
        ...
        int hour  = root.get("Hour",  -1).asInt();
        int min   = root.get("Minute",-1).asInt();
        int sec   = root.get("Second",-1).asInt();

        // -------------------------------------------------------
        // 2. Convert to wxDateTime
        // -------------------------------------------------------
        wxDateTime t;
        if (day >= 1 && month >= 0 && year >= 0)
        {
            t.Set(day, (wxDateTime::Month)month, year, hour, min, sec);
        }
        else
        {
            // GRIB sends -1 when timeline is cleared
            t = wxDateTime(); // invalid
        }

        // -------------------------------------------------------
        // 3. Convert wxDateTime → climatology month index
        // -------------------------------------------------------
        int climatologyMonth = t.IsValid() ? t.GetMonth() : 0;

        // -------------------------------------------------------
        // 4. Get current render params from dialog
        // -------------------------------------------------------
        ClimatologyRenderParams p = m_pClimatologyDialog->GetRenderParams();

        // -------------------------------------------------------
        // 5. Update timeline fields
        // -------------------------------------------------------
        p.currentMonth      = climatologyMonth;
        p.nextMonth         = climatologyMonth;
        p.monthInterpolation = 1.0;   // GRIB timeline is discrete

        // -------------------------------------------------------
        // 6. Push into dialog (UI + StandardDisplayParams + RenderParams)
        // -------------------------------------------------------
...
// 1. Day of year GRIB sync (instead of month only) GRIB already sends full date/time. 
// You simply convert it to DOY and push it into your unified cyclone timeline fields. 
// Drop in replacement for your GRIB_TIMELINE handler:
...
if (message_id == "GRIB_TIMELINE")
{
    ...
    if (day >= 1 && month >= 0 && year >= 0)
        ...
        t = wxDateTime();   // GRIB cleared timeline

    int doy = t.IsValid() ? t.GetDayOfYear() : 0;

    ClimatologyRenderParams p = m_pClimatologyDialog->GetRenderParams();

    // DOY sync (new unified model)
    p.centerDay        = doy;
    p.currentMonth     = t.IsValid() ? t.GetMonth() : 0;
    p.nextMonth        = p.currentMonth;
    p.monthInterpolation = 1.0;
...
bool climatology_pi::RenderGLOverlay(wxGLContext* pcontext, PlugIn_ViewPort* vp)
{
    ...
    ClimatologyRenderParams p =
        m_pClimatologyDialog->BuildRenderParams(vp, nullptr);
                                      ...
                                      wxString& message_body)
{
    if (message_id == "GRIB_TIMELINE")
    {
        ...
        ClimatologyRenderParams p = m_pClimatologyDialog->GetRenderParams();
        p.currentMonth = t.IsValid() ? t.GetMonth() : 0;
        p.centerDay    = t.IsValid() ? t.GetDayOfYear() : 0;
