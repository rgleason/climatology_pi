...
if [ -n "${OCPN_TARGET}" ]; then
    tarball_name=climatology_pi-1.6.48.0-${PKG_TARGET}-x86-wx32-${PKG_TARGET_VERSION}-${OCPN_TARGET}-tarball
else
    tarball_name=climatology_pi-1.6.48.0-${PKG_TARGET}-x86-wx32-${PKG_TARGET_VERSION}-tarball
fi
...
if [ "$BUILD_BRANCH_LOWER" = "master" ]; then
    ...
    if [ -n "$BUILD_TAG"  ]; then
        ...
        VERSION="1.6.48.0+${BUILD_ID}.${commit}"
...
# Only upload if the owner did the pull request or a member of $COLLABORATORS.
    if [ "$CIRCLE_PROJECT_USERNAME" = "$CIRCLE_USERNAME" ] || [[ -n "${collab_users+1}"  && ! -z ${CIRCLE_USERNAME}  && "$collab_users" =~ "$git_user" ]]; then
        cloudsmith push raw --republish --no-wait-for-sync \
            --name climatology_pi-1.6.48.0-msvc-x86-wx32-10.0.26200-${OCPN_TARGET}-metadata \
            --version ${VERSION} \
            --summary "climatology opencpn plugin metadata for automatic installation" \
            $REPO $xml

        cloudsmith push raw --republish --no-wait-for-sync \
            --name $tarball_name \
            --version ${VERSION} \
            --summary "climatology opencpn plugin tarball for automatic installation" \
            $REPO $tarball

        if [ "${PKG_EXT}" != "gz" ] && [ "$pkg" != "" ]; then
            cloudsmith push raw --republish --no-wait-for-sync \
                --name opencpn-package-climatology-1.6.48.0-msvc-x86-wx32-10.0.26200-${OCPN_TARGET}.${PKG_EXT} \
                --version ${VERSION} \
                --summary "climatology .${PKG_EXT} installation package" \
                $REPO $pkg
        fi
    fi
fi
